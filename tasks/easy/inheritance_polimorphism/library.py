...
class LibraryReader(Person):
    ...
    def take_books(self, *args):
        ...
        if len(args) < 4:
            ...
            return f"{self.fullname} взяла 4 книги"

    def return_book(self, *args):
        not_return_book = set(args).difference(self.books)
        if not not_return_book:
            if len(args) < 4:
                self.books = self.books.difference(set(args))
                return f"{self.fullname} вернул(а) книги: {','.join(sorted(args))}"
            else:
                raise ValueError(f"{self.fullname} не брал:{','.join(sorted(not_return_book))}")
